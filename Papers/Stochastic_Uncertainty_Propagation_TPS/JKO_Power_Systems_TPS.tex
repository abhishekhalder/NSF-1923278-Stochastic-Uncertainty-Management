...
was established in \cite{duong2014conservative}. To numerically compute \eqref{KineticProx}, we employ the proximal algorithm proposed in \cite{caluya2019TAC} with finite samples as explained next.
...
The proximal algorithm recursively updates probability-weighted scattered point cloud $\{\bm{x}_{k}^{i},\tilde{\varrho}_{k}^{i}\}_{i=1}^{N}$ where 
