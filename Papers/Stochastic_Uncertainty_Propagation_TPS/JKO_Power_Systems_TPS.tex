...
\begin{algorithmic}[1]
\Procedure{ProxRecursion}{$\bm{\varrho}_{k-1}, \tilde{\bm{x}}_{k-1}, \tilde{\bm{x}}_{k}, h, \varepsilon, N, \delta, L$}
\For{$i=1$ to $N$}
...
For completeness, the proximal algorithm for updating $\{\tilde{\varrho}_{k-1}^{i}\}_{i=1}^{N}\mapsto\{\tilde{\varrho}_{k}^{i}\}_{i=1}^{N}$ is outlined in Algorithm \ref{algo:KineticProx}. As shown in Fig. \ref{fig:BlockDiagm}, this algorithm, at a conceptual level, takes the pre and post update state samples 
...
and $\{\tilde{\varrho}_{k-1}^{i}\}_{i=1}^{N}$ as inputs. The Algorithm \ref{algo:KineticProx} also needs input parameters $h,\varepsilon,N,\delta,L$. Specifically, $h$ is the time-step size, $\varepsilon$ is    


Its convergence guarantees can be found in \cite[Sec. III.C]{caluya2019TAC}.
