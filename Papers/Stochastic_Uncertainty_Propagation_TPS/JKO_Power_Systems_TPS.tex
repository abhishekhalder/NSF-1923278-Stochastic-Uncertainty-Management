...
Thus, $\tilde{\varrho}_{k}^{i}$ is the joint PDF value obtained from \eqref{KineticProx} at $\tilde{\bm{x}}_{k}^{i}$, the $i$th (transformed) state sample at the $k$th time step. The schematic of the algorithm is shown in Fig. \ref{fig:BlockDiagm}. 
...
For completeness, the proximal algorithm for updating $\{\tilde{\varrho}_{k-1}^{i}\}_{i=1}^{N}\mapsto\{\tilde{\varrho}_{k}^{i}\}_{i=1}^{N}$ is outlined in Algorithm \ref{algo:KineticProx}. As shown in Fig. \ref{fig:BlockDiagm}, at a conceptual level, this algorithm takes the pre and post update state samples 
...
and $\{\tilde{\varrho}_{k-1}^{i}\}_{i=1}^{N}$ as inputs. The Algorithm \ref{algo:KineticProx} needs additional inputs $h,\varepsilon,N,\delta,L$, which are parameters internal to the algorithm. Specifically, $h$ is the time-step size   
