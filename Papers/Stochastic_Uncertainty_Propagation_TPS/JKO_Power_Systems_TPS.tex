...
$$\tilde{\varrho}_{k}(\bm{\xi},\bm{\eta}) \xrightarrow{h\downarrow 0} \tilde{\varrho}(t=kh,\bm{\xi},\bm{\eta}),$$
was established in \cite{duong2014conservative}. To numerically compute \eqref{KineticProx}, we employ the proximal algorithm proposed in \cite{caluya2019TAC} with finite number od samples, as explained next.
...
Following \cite{caluya2019TAC}, our idea to numerically solve \eqref{KineticProx} is to recursively update probability-weighted scattered point cloud $\{\bm{x}_{k}^{i},\tilde{\varrho}_{k}^{i}\}_{i=1}^{N}$ where 
$$\bm{x}_{k}^{i}\equiv\left(\bm{\xi}_{k}^{i},\bm{\eta}_{k}^{i}\right)^{\!\top}, \quad i=1,\hdots,N.$$
