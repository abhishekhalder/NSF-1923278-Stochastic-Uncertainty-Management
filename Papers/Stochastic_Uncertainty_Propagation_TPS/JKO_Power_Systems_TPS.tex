...
We notice that \eqref{XiEtaVectorSDE} is a mixed-conservative SDE with \emph{isotropic} degenerate diffusion. In particular, the pushforward of the known initial joint PDF \eqref{FPKInitCond} via $\bm{\Psi}$, is given by
\begin{align}
...
&\tilde{\rho}(t=t_{0},\bm{\xi},\bm{\eta}) = \underbrace{\tilde{\rho}_{0}(\bm{\xi},\bm{\eta})}_{\text{from}\;\eqref{PushforwardIC}}. \label{NewIC}	
\end{align}
...
Since \eqref{NewPDE} is a standard kinetic Fokker-Planck PDE with isotropic degnerate diffusion, our strategy is to perform a proximal recursion of the form \eqref{ProxRecursionInfiniteDim} for \eqref{TransformedPDEivp}, and then to pushforward the resulting joint PDFs via $\bm{\Psi}^{-1}$ to the original $(\bm{\theta},\bm{\omega})$ state space.
