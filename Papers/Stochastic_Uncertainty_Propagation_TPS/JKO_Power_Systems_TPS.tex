...
& \\
tangent of phase shift & $\tan\varphi_{ij}\in{\rm{Unif}}\left([0,0.25]\right)$\\
& \\
mechanical power input & $P_{i}^{\text{mech}}\in{\rm{Unif}}\left([0,10]\right)$\\
& \\
coupling coefficient & $k_{ij}\in{\rm{Unif}}\left([0.7,1.2]\right)$\\
& \\
\hline
\end{tabular}
\caption{Parameters used for the numerical simulation reported in Sec. \ref{subsec:NumSimSynthetic}.}
...
\begin{align*}
\rho_{0}\equiv\rho(t=0,\theta(0),\omega(0)) = &\;{\rm{Unif}}\left([0,2\pi)^{n}\;\text{rad}\right)\\
	&\qquad\times {\rm{Unif}}\left([-0.1,0.1]\;\text{rad/s}\right),
\end{align*}
and the stochastic parameters as in Table \ref{table:ParamUncertaintiesSynthetic}.
