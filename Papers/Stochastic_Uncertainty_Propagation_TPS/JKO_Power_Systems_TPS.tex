...
Following \cite{caluya2019TAC}, we solve \eqref{KineticProx} by recursively updating the probability-weighted scattered point clouds $\{\bm{x}_{k}^{i},\tilde{\varrho}_{k}^{i}\}_{i=1}^{N}$ where 
...
In the numerical simulations reported in Sec. \ref{sec:NumericalSimulations}, the states $\{\bm{x}_{k}^{i}\}_{i=1}^{N}$ are updated by the Euler-Maruyama scheme applied to (\ref{ItoSDEvectorlevel}). If one wishes so, the Euler-Maruyama scheme in Fig. \ref{fig:BlockDiagm} may be replaced by other SDE integrators, see e.g., \cite[Sec. III.B.2, Remark 1]{caluya2019TAC}. 
...
\begin{algorithm}[t]
\caption{Proposed proximal algorithm for $\bm{\varrho}_{k-1}\mapsto\bm{\varrho}_{k}$}
\label{KineticProxAlgorithm}
\begin{algorithmic}[1]
\Procedure{ProxRecursion}{$\bm{\varrho}_{k-1}, h, \varepsilon, N, \delta, L$}
\For{$i=1$ to $N$}
...
\State $\bm{y} \gets \left[\bm{\varrho}_{k-1}\oslash\left(\bm{\Gamma}_{k}\bm{z}_{0}\right), \bm{0}_{N\times(L-1)}\right]$ \Comment{initialize}
\State $\ell = 1$
\While{$\ell\leq L$}
\State $\bm{z}(:,\ell+1) \gets \left(\bm{\zeta}_{k-1}\oslash\left(\bm{\Gamma}_{k}^{\top}\bm{y}(:,\ell)\right)\right)^{\frac{1}{1+2\varepsilon/h}}$
\State $\bm{y}(:,\ell+1) \gets \bm{\varrho}_{k-1} \oslash \left(\bm{\Gamma}_{k}\bm{z}(:,\ell+1)\right)$
...
\EndIf
\EndWhile\\
\Return $\bm{\varrho}_{k} \gets \bm{z}(:,\ell) \odot \left(\bm{\Gamma}_{k}^{\top}\bm{y}(:,\ell)\right)$ \Comment{proximal update}
\EndProcedure	
\end{algorithmic}
\end{algorithm}
...
For completeness, the overall proximal algorithm is {\red{given}}. Its convergence guarantees can be found in \cite[Sec. III.C]{caluya2019TAC}.
