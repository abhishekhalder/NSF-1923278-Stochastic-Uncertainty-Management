...
To numerically compute \eqref{KineticProx}, we employ the proximal algorithm proposed in \cite{caluya2019TAC} with finite $N$ samples. Specifically, the proximal algorithm recursively updates probability-weighted scattered point cloud $\{\bm{x}_{k}^{i},\tilde{\varrho}_{k}^{i}\}_{i=1}^{N}$.
