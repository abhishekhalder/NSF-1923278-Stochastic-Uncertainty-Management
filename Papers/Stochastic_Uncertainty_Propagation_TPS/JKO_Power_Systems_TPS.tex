...
\begin{remark}
We clarify here that the network-reduced model \eqref{ItoSDEcomponentlevel} is obtained from the so-called structure preserving power network model \cite{odun2012structure} with $n$ generators, after applying the Kron reduction, and therefore, has all-to-all connection topology. If the unreduced network has $n$ generators and $m$ buses, then the unreduced admittance matrix $\bm{Y}_{\rm{unreduced}} \in \mathbb{C}^{(n+m)\times(n+m)}$ satisfies
\begin{align}
\bm{Y}_{\rm{unreduced}} = .
\label{unreducedY}	
\end{align}
The network reduced admittance matrix $\bm{Y}\in\mathbb{C}^{n\times n}$ is the Schur complement
\[\bm{Y} = \bm{Y}_{\rm{unreduced}}.\]	
\end{remark}
...
Fig. \ref{fig:CompTimeSynthetic} highlights that for computing high dimensional transient joint state PDFs as in here, the proposed variational framework enjoys remarkably fast computational time. Notice that constructing histograms or other conventional density estimators for the joint state PDF by making a grid over the 40 dimensional state space, is computationally prohibitive. 
