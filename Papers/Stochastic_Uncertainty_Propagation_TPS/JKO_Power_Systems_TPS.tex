...
\thanks{A. Halder and K.F. Caluya are with the Department of Applied Mathematics, University of California, Santa Cruz, CA 95064, USA, {\texttt{\{ahalder,kcaluya\}@ucsc.edu}}.}% <-this % stops a space
>>>>>>> 1fdb1f70e0079d2abe275d3bdca9cc245852d511
...
\caption{\small{Time evolution of the $(\theta_{i},\omega_{i})$ bivariate marginal PDFs of the generator $i=1,\hdots,5$, for the Kron-reduced IEEE 14 bus simulation setup described in Sec. \ref{subsec:NumSimIEEEBus}. The $5$ rows above correspond to the $5$ generator nodes; these are buses 1, 2, 3, 6 and 8 in Fig. \ref{fig:Motivating}. The columns correspond to the time snapshots. The colorbar at the bottom shows the marginal PDF values. The $\theta_i$ are in rad, the $\omega_i$ are in rad/s.}}
\vspace*{-0.1in}
...
Following Sec. \ref{subsec:samplepathdynamics}, we consider the Kron-reduced dynamics \eqref{ItoSDEcomponentlevel} for the IEEE 14 bus system shown in Fig. \ref{fig:Motivating}. In this case, we have $n=5$ nodes which correspond to the buses 1, 2, 3, 6 and 8 in Fig. \ref{fig:Motivating}.
{\red{The original parameters of the 14-bus system are from Matpower \cite{zimmerman2010matpower}. The calculation of network reduced admittance matrices in \eqref{DefParam} is described in \ref{subsec:samplepathdynamics} and \cite{dorfler2012kron}.
The parameters $m_{i}, \gamma_{i}$ are obtained from ANDES \cite{cui2021andes}.}}
We randomly generated $\sigma_{i}\in [1,5]$ as
...
Fig. \ref{fig:IEEE14marginals} shows the time snapshots of the $(\theta_{i},\omega_{i})$ bivariate marginal PDFs for generator $i=1,\hdots, n=5$, computed using the weighted point clouds $\{\bm{x}_{k}^{i},\varrho_{k}^{i}\}_{i=1}^{N}$. Notice that in Fig. \ref{fig:IEEE14marginals}, the horizontal axes of the subfigures have periodic boundary conditions, i.e., $(\theta_{i},\omega_{i})\in\mathbb{T}\times\mathbb{R}$ (cylinder). Fig. \ref{fig:CompTimeIEEE14bus} shows the computational times needed to update $\{\bm{x}_{k}^{i},\varrho_{k}^{i}\}_{i=1}^{N}$ via the algorithm in Sec.  \ref{sec:ProxAlgo}.  
...
R.D.~Zimmerman, C.E.~Murillo-S{\'a}nchez, R.J. Thomas, ``MATPOWER: Steady-state operations, planning, and analysis tools for power systems research and education'', \emph{IEEE Transactions on power systems},  Vol. 26, No. 1, pp. 12--19, 2010.
