...
Our numerical simulations for the IEEE 14 bus system consider the following three cases:

\noindent\textbf{Case I.} Nominal case where ...

\noindent\textbf{Case II.} post-contingency simulation after the line 13 fails.

\noindent\textbf{Case III.}	load data are time-varying ...

 

For all the three cases, we used the randomly generated noise coefficients $\sigma_{i}\in [1,5]$ as
...
Fig. \ref{fig:IEEE14omegamarginalsCase12} shows the corresponding univariate $\omega_{i}$-marginal PDFs. In particular, Fig. \ref{fig:IEEE14omegamarginalsCase12}(c) helps compare the $\omega_{4}$ marginal evolution for bus 6 in the nominal (Case I) and post-fault (Case II) scenarios. Since Case II considers the failure of line 13, we expect from Fig. \ref{fig:Motivating} that the transient effects of this fault will be prominent for bus 6. Indeed, Fig. \ref{fig:IEEE14omegamarginalsCase12}(c) shows that although the $\omega_{4}$ marginals in both Case I and II, at all times have highest probability around $0$ rad/s, the post-fault marginals show larger dispersion.  
