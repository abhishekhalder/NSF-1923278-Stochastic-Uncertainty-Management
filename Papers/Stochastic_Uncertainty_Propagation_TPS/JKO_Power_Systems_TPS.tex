...
\begin{subequations}
\begin{align}
 P_{i} &= P_{i}^{\text{mech}} - P_{i}^{\text{load}} - |V_{i}|^{2} \Re\left(Y_{ii}\right) + \Re\left(V_i \cdot I_{i}^{*}\right),\\
 \varphi_{ij} &= \begin{cases} -\arctan\left(\dfrac{\Re\left(Y_{ij}\right)}{\Im\left(Y_{ij}\right)}\right), & \text{if}\;i\neq j,\\
 0, & \text{otherwise},
 \end{cases}\\
 k_{ij} &= \begin{cases} |V_{i}| |V_{j}| \vert Y_{ij}\vert, & \text{if}\;i\neq j,\\
 0, & \text{otherwise},
 \end{cases}
\end{align}
\end{subequations}
where $P_{i}^{\text{mech}}$ is the mechanical power input, $P_{i}^{\text{load}}$ is {\red{the real load (ignoring reactive load)}}, and $|V_{i}|$ is the voltage magnitude for generator $i$. {\red{{\textbf{Xinbo:}} One sentence saying any approximation made in (2) by using $V_{i}$ instead of the internal voltage $E_{i}$}}
...
\begin{remark}
If the unreduced network has $n$ generators and $m$ buses, then the unreduced admittance matrix $\bm{Y}_{\rm{unreduced}} \in \mathbb{C}^{(n+m)\times(n+m)}$ satisfies
\begin{align}
\bm{Y}_{\rm{unreduced}} = .
\label{unreducedY}	
\end{align}
