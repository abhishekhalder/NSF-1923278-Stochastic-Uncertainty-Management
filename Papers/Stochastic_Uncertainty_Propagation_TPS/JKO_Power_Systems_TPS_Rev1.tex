...
The algorithmic appeal of the proximal recursion \eqref{ProxRecursionInfiniteDim} is that it opens up the possibility to compute the solution of the PDE initial value problem via recursive convex minimization. A point cloud-based proximal algorithm was proposed in \cite{caluya2019ACC,caluya2019TAC} which was reported to have very fast runtime. {\blue{The main idea in these references was to \emph{co-evolve} the time-varying state samples (via numerical SDE solver) as well as the joint PDF values (via \eqref{ProxRecursionInfiniteDim}) evaluated at those samples. The resulting computation is an \emph{online} propagation of the joint PDFs as opposed to the computation in Monte Carlo or density estimation methods \cite{silverman1998density}. The latter methods only propagate the state samples and then approximate the joint PDFs as \emph{offline} post-processing.}} 
