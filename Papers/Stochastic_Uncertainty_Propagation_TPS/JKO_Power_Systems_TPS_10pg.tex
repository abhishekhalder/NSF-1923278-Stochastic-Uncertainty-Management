...
\begin{figure}
\centering
\includegraphics[width=0.8\linewidth]{UncPropBlockDiagm.pdf}
...
\begin{algorithmic}[1]
\Procedure{Prox}{$\bm{\varrho}_{k-1}, \tilde{\bm{x}}_{k-1}, \tilde{\bm{x}}_{k}, h, \varepsilon, N, \delta, \ell_{\max}$}
\For{$i=1$ to $N$}
...
\caption{\small{Comparison of the $(\theta_{4},\omega_{4})$ marginals for bus 6 in the nominal (\textbf{Case I}) and post-fault (\textbf{Case II}) scenarios in the IEEE 14 bus simulation described in Sec. \ref{subsec:NumSimIEEEBus}.}}
\vspace*{-0.2in}
...
\caption{\small{Comparison of the $\omega_{4}$ marginals for bus 6 in the nominal (\textbf{Case I}) and post-fault (\textbf{Case II}) scenarios in the IEEE 14 bus simulation described in Sec. \ref{subsec:NumSimIEEEBus}.}}
\vspace*{-0.1in}
\label{fig:IEEE14omegamarginalsCase12}
\end{figure}
...
With step size $h=10^{-3}$, we discretized time $t\in[0,1]$. Using the algorithm detailed in Sec. \ref{sec:ProxAlgo}, for both cases, we propagated $N=1000$ random samples from \eqref{rho0IEEE14}, and evaluated at \eqref{rho0IEEE14}, to update the scattered weighted point clouds $\{\bm{x}_{k}^{i},\varrho_{k}^{i}\}_{i=1}^{N}$ over $\mathbb{T}^{5}\times\mathbb{R}^{5}$ at times $t_{k}:=kh$, $k\in\mathbb{N}$. In Algorithm \ref{algo:KineticProx}, we used the algorithmic parameters $\varepsilon=0.05$, $\delta=10^{-3}$, $\ell_{\max}=300$.
...
%{\red{That the generator 4 states in Case II have larger uncertainties than in Case I, is also visible in the bivariate marginal plots if we compare the fourth row in Fig. \ref{fig:IEEE14marginalsCase0} with the same in Fig. \ref{fig:IEEE14marginalsCase1}.}} 




Fig. \ref{fig:CompTimeIEEE14bus} shows the computational times for updating the point clouds $\{\bm{x}_{k}^{i},\varrho_{k}^{i}\}_{i=1}^{N}$ via the algorithm in Sec. \ref{sec:ProxAlgo} for \textbf{Cases I} and \textbf{II}.  
...
% expanded version of this paper (exceeds 10 pages)

\bibitem{halder2021stochasticExpanded}
