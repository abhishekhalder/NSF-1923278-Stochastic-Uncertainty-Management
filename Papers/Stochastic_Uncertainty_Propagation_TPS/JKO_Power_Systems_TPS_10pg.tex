...
The proposed computational framework can be applied as is when $P_{i}^{\rm{mech}}, P_{i}^{\rm{load}}$, and thus $P_{i}$ in \eqref{defPi}, are bounded time-varying functions. {\red{This can be the case, for instance, when ...}} 	
\end{remark}
...
\noindent\textbf{Case I.} {\red{Nominal case where ...}}

\noindent\textbf{Case II.} {\red{post-contingency simulation after the failure of line 13 (see Fig. \ref{fig:Motivating}).}}

%\noindent\textbf{Case III.}	load data are time-varying ...

 

For all the cases mentioned above, we used the randomly generated noise coefficients $\sigma_{i}\in [1,5]$ as
