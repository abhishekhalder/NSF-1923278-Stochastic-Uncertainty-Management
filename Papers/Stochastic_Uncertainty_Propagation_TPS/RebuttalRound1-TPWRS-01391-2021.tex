...
\ul{Furthermore, Fig. 7 in the revised manuscript has also been expanded to include the univariate angular velocity transient marginal plots for all the five generators of the IEEE 14 bus system; previous version had this plot only for the generator 4 (bus 6)}.

{In addition to the above, \ul{we have included the following new figures in the revised manuscript}: Figs. 8, 10, 13, 14. In particular, Fig. 8 addresses comment \textbf{R2.5}. Fig. 10 addresses comment \textbf{R1.7}. Figs. 13 and 14 address comments \textbf{R1.4, R4.2 and R4.7}.
}}
...
{\nib{ \blue \ul{Regarding the comparison with existing methods}, we have provided comparison with Monte Carlo in the revised manuscript's Figs. 12, 13 and 14; \ul{among these Figs. 13 and 14 are new} (in response to comment \textbf{R4.7}). Also, the \ul{new Fig. 8 in the revised manuscript shows the consistency for the rotor angle stochastic predictions w.r.t. Monte Carlo for the simulation reported in Sec. V-A}, as requested by comment \textbf{R2.5}. Please also see our response to comment \textbf{R4.2} where we explain why propagating the joint PDF for even a modest 5 generator system with discretization-based PDE solvers such as finite element simulation is challenging with general purpose computers.}}
...
{\nib{ \blue Please note that our joint PDF computation has no truncation error accumulation issue (as in for example, moment based propagation techniques where such errors arise from a priori assuming finite dimensionality of the sufficient statistic). \ul{Furthermore, the new Fig. 10 reports the total computational time for 1 minute time horizon}. Please also see the additional error statistics included in the revised version's Fig. 13 and Fig. 14.}}
...
{\nib{ \blue Thanks. \ul{We have included new Fig. 10 which reports the total computational time for 1 minute time horizon} for all the three simulation cases in Sec. V-A and V-B. For each of these cases, Fig. 10 shows the total computational time across 100 simulation instances. This Fig. 10 is referred to in text in Sec. V-A (last paragraph), and also in Sec. V-B (third paragraph, last sentence).

While showing the per-step computational times in Figs. 9 and 11 in the revised manuscript are somewhat unconventional, in the revised manuscript's Sec. V-A end, \ul{we have added detailed explanation (highlighted in blue)} of why such depictions are meaningful in our computation.}}
...
{\nib \blue{Thanks for this comment. \ul{The revised manuscript's newly incorporated Fig. 8 shows the statistical uncertainty bounds for the rotor angle sample paths for the simulation in Sec. V-A}. The inline text citing this figure in Sec. V-A is highlighted in blue.}}
...
{\nib \blue{\ul{Regarding the comparison with existing methods}, we have provided comparison with Monte Carlo in the revised manuscript's Figs. 12, 13 and 14; \ul{among these Figs. 13 and 14 are new} (in response to comment \textbf{R4.7}). Also, the \ul{new Fig. 8 in the revised manuscript shows the consistency for the rotor angle stochastic predictions w.r.t. Monte Carlo for the simulation reported in Sec. V-A}, as requested by comment \textbf{R2.5}. 

\ul{Please note that performing finite element simulation for 5-generator system is a very tall order since the state space then is 10 dimensional, and even if we do a very coarse 10 discretizations per dimension, then such a simulation would require us to evolve a joint PDF over $10^{10}$ grid points}. This requires dedicated high-performance computing architecture and an undertaking well beyond the scope of this paper. This also explains why the state-of-the-art for direct computation of transient joint PDF in power systems literature remains single-machine-infinite-bus $(n=1)$ as in the 2013 ref. [45] which indeed performed the finite element simulation with a two dimensional discretization, and mentioned the difficulty of scaling this computational approach for even moderately large sized systems.

\ul{Regarding the computational time for longer horizon}, we have included new Fig. 10 which reports the total computational time for 1 minute time horizon for all the three simulation cases in Sec. V-A and V-B. For each of these cases, Fig. 10 shows the total computational time across 100 simulation instances. This Fig. 10 is referred to in text in Sec. V-A (last paragraph), and also in Sec. V-B (third paragraph, last sentence).}}
...
{\nib \blue{\ul{In the revised manuscript, we have incorporated new Fig. 13 and Fig. 14}. Fig. 13 shows the relative error between the covariances of the Monte Carlo and proximal joint state PDFs, as measured w.r.t. the normalized Bures-Wasserstein distance. \ul{We have explained the same in text in the last but one paragraph of Sec. V-B (highlighted in blue)}. Fig. 14 shows the absolute error between the Monte Carlo and proximal joint state PDFs, as measured w.r.t. the 2-Wasserstein metric. \ul{We have explained the same in text in the last paragraph of Sec. V-B (highlighted in blue, with added inline citations)}.}}
